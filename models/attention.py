...
class NonLocalSparseAttention(nn.Module):
    def __init__( self, n_hashes=4, channels=64, k_size=3, reduction=4, chunk_size=144, max_hash_buckets=128, res_scale=1):
        super(NonLocalSparseAttention,self).__init__()
        self.chunk_size = chunk_size
        self.n_hashes = n_hashes
        self.reduction = reduction
        self.max_hash_buckets = max_hash_buckets
    ...
    def forward(self, input):
        ...
        hash_buckets = min(L//self.chunk_size + (L//self.chunk_size)%2, self.max_hash_buckets) #default 128
