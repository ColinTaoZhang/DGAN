...
checkpoint = torch.load('./checkpoints/Denoising/models/Unet_G_DN/model_best.pth')
...
optimizer_DM = optim.Adam(model_demosaic.parameters(), lr=new_lr, betas=(0.9, 0.999),eps=1e-8, weight_decay=1e-8)
...
if warmup:
    ...
    scheduler_cosine_DM = optim.lr_scheduler.CosineAnnealingLR(optimizer_DM, opt.OPTIM.NUM_EPOCHS-warmup_epochs, eta_min=1e-6)
    scheduler_DM = GradualWarmupScheduler(optimizer_DM, multiplier=1, total_epoch=warmup_epochs, after_scheduler=scheduler_cosine)
    scheduler_DM.step()

if len(device_ids)>1:
    model_restoration = nn.DataParallel(model_restoration, device_ids = device_ids)
    model_demosaic = nn.DataParallel(model_demosaic, device_ids = device_ids)
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):    
        ...
        for param in model_restoration.parameters():
            param.grad = None
        for param in model_demosaic.parameters():
            param.grad = None

        target = data[0].cuda()
        input_ = data[1].cuda()
        # target_DN = data[4].cuda()

        if epoch < pretrain_epoch:
            with torch.no_grad():
                denoised, denoised_green = model_restoration(input_)
        else:
            denoised, denoised_green = model_restoration(input_)
        restored, restored_green = model_demosaic(denoised, denoised_green)
        
        loss = criterion(restored, target) + (30-epoch*27.0/100.0)*criterion(restored_green, torch.cat((target[:,1:2], target[:,3:4]), dim=1))
    
        loss.backward()
        if not epoch < pretrain_epoch:
            optimizer.step()
        optimizer_DM.step()
        ...
        if i%eval_now==0 and i>0:
            ...
            with torch.no_grad():
                ...
                for ii, data_val in enumerate((val_loader), 0):
                    ...
                    # restored = model_demosaic(model_restoration(input_))
                ...
                if psnr_val_rgb > best_psnr:
                    ...
                    torch.save({'epoch': epoch, 
                                'state_dict_DN': model_restoration.state_dict(),
                                'state_dict_DM': model_demosaic.state_dict(),
